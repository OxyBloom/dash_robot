  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  try {
    ...
    RCLCPP_ERROR(rclcpp::get_logger("DiffDriveArduinoHardware"), 
                 "Error parsing integer parameters: %s", e.what());
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
  ...
  // wheel_l_.pos = 0.0;  // or keep incrementing based on commands
  // wheel_l_.vel = wheel_l_.cmd;

  // pos_prev = wheel_r_.pos;
  // wheel_r_.pos = 0.0;  // or keep incrementing based on commands
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  double vel_to_pwm_factor = 255.0 / 5.0;  // Assuming max 5 rad/s = 255 PWM
  
  int motor_l_pwm = (int)(wheel_l_.cmd * vel_to_pwm_factor);
  int motor_r_pwm = (int)(wheel_r_.cmd * vel_to_pwm_factor);
  
  // Clamp to PWM limits
  motor_l_pwm = std::max(-255, std::min(255, motor_l_pwm));
  motor_r_pwm = std::max(-255, std::min(255, motor_r_pwm));
  
  // Use motor raw PWM command instead of encoder-based control
  comms_.set_motor_values(motor_l_pwm, motor_r_pwm);
  // RCLCPP_INFO(rclcpp::get_logger("DiffDriveArduinoHardware"), "Motor commands: L=%d R=%d", motor_l_pwm, motor_r_pwm);
  return hardware_interface::return_type::OK;
}
